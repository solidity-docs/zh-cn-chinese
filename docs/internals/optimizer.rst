...
最终的程序应该采用 *表达式分割形式*，在这种形式下（除了循环条件之外），
函数调用不能嵌套在表达式中，
并且所有函数调用的参数必须是变量。
...
完全内联通过函数体替换某些函数的特定调用。
在大多数情况下，这并没有太多帮助，
因为它只会增加代码大小而没有实际好处。此外，
代码通常很昂贵，我们通常更喜欢更短的代码而不是更高效的代码。
然而，在某些情况下，内联函数可能对后续的优化步骤产生积极影响，
例如，如果其中一个函数参数是常量。
...
建议将函数提升器和表达式拆分器作为先决条件，因为它们可以使步骤更高效，
但对于正确性来说并非必需。
特别是，具有其他函数调用作为参数的函数调用不会进行内联，
但在运行表达式拆分器之前可以确保输入中没有此类调用。
