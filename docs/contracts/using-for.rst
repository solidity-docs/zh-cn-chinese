...
指令 ``using A for B`` 可用于将函数（ ``A``）
作为运算符附加到用户定义的值类型
或作为成员函数附加到任何类型（ ``B``）。
成员函数将调用它们的对象作为第一个参数
（类似于 Python 中的 ``self`` 变量）。
运算符函数将接收操作数作为参数。
...
请注意，注意，只有当 ``using for`` 位于库合约内时，才能指定私有库函数。
...
这意味着如果传递内存或值类型，即使是 ``self`` 变量，也会执行复制。
只有在使用存储引用变量或调用内部库函数时，才不会执行复制。

另一个示例展示了如何为用户定义的类型定义自定义操作符：
